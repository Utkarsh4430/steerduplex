...
# Make `src/` importable when this module is launched as a standalone script
# (e.g. via subprocess.Popen from launch_moshi_servers.py, which does not
# preset PYTHONPATH). parents[1] from src/moshi_server/moshi_server.py
# resolves to src/, which hosts the `inference` and `moshi_server` packages.
import pathlib as _pathlib
_SRC = str(_pathlib.Path(__file__).resolve().parents[1])
...
# Canonical personaplex_loader lives under src/inference/ (also used by
# inference/generate.py). Import from there so there's a single source of
# truth across the inference and server entry points.
