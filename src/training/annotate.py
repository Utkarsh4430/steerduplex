"""Whisper-based audio annotation for word-level alignments.

Adapted from moshi-finetune's annotate.py. Generates .json files alongside
each .wav file with word-level timestamps for text/audio interleaving.

Usage:
    # Via format_dataset.py (automatic)
    python -m pipeline.format_dataset --config configs/generation.yaml

    # Direct invocation
    python -m training.annotate data/formatted/_temp_manifest.jsonl --lang en --local
"""
...
def load_audio_paths(egs_path: Path) -> list[Path]:
    """Load audio paths from a JSONL manifest file."""
...
def process_one(
    ...
    """Transcribe one audio file and write word-level alignments to JSON."""
...
def main():
    parser = argparse.ArgumentParser(
        description="Annotate audio with word-level timestamps via Whisper."
    )
    parser.add_argument("egs", type=Path, help="Path to JSONL manifest file")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Whisper model size (medium recommended for stereo).",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--keep_silence_in_segments", type=bool, default=True,
        help="Keep silence at segment boundaries for better alignment.",
    )
    parser.add_argument(
        "-l", "--local", action="store_true", help="Run locally (no SLURM)."
    )
    ...
    if args.local:
        ...
        try:
            ...
            print("submitit not installed. Use --local for local execution.")
            sys.exit(1)

        log_folder = Path.home() / "tmp" / "steerduplex_annotate"
        log_folder.mkdir(parents=True, exist_ok=True)
        executor = submitit.SlurmExecutor(folder=log_folder)
        executor.update_parameters(
            cpus_per_task=6, ntasks_per_node=1, gpus_per_node=1,
            time=60 * 24 * 10, signal_delay_s=30,
            stderr_to_stdout=True, array_parallelism=1000,
            job_name="steerduplex_annotate",
        )
        jobs = []
        with executor.batch():
